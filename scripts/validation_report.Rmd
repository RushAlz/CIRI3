...
# Normalise column order so samples line up between joint and decoupled
common_samples <- intersect(colnames(joint_BSJ)[-1], colnames(decoupled_BSJ)[-1])
stopifnot(length(common_samples) > 0)
joint_BSJ     <- joint_BSJ[,     c("circRNA_ID", common_samples)]
joint_FSJ     <- joint_FSJ[,     c("circRNA_ID", common_samples)]
decoupled_BSJ <- decoupled_BSJ[, c("circRNA_ID", common_samples)]
decoupled_FSJ <- decoupled_FSJ[, c("circRNA_ID", common_samples)]
...
) |> kable(caption = "Matrix dimensions")
```
